...
function resolveDatabase(raw, env, dockerService) {
  const database = raw.database && typeof raw.database === "object" ? raw.database : {};
  const runner = database.runner === "docker" ? "docker" : "psql";
...
export function loadConfig(projectRoot, env = null) {
  ...
  config.database = resolveDatabase(raw, env, dockerService);
  return config;
}
...
export function applyRunnerOverride(config, runner) {
  if (!runner) return config;
  if (runner !== "psql" && runner !== "docker") {
    throw new Error(`Unknown database runner: ${runner}`);
  }
  return {
    ...config,
    database: {
