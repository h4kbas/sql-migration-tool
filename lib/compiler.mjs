...
const BLOCK_KINDS_BY_MODE = {
  init: new Set(["model"]),
  seed: new Set(["seed", "defer"]),
  "migrate:up": new Set(["migration_up", "defer"]),
};

function collectUnits(parsedFiles, mode, includeFreestanding) {
  const units = [];
  let index = 0;
  ...
  for (const parsed of parsedFiles) {
    ...
    for (const block of parsed.blocks) {
      ...
      if (block.kind === "defer" && (block.dependsOn ?? []).length === 0) {
...
function resolveUnits(units, appliedNames) {
  ...
  while (pending.length > 0) {
    ...
    for (const unit of pending) {
      ...
      if (unit.type === "defer" || unit.type === "migration_up") {
...
function renderResolvedUnits(resolved) {
  const parts = [];

  for (const unit of resolved) {
    if (unit.type === "migration_insert") {
      parts.push(`INSERT INTO api.migration (name) VALUES (${sqlLiteral(unit.name)});\n`);
      continue;
    }
...
export function compileInit(parsedFiles) {
  const parts = [];
...
export function compileSeed(parsedFiles, appliedNames = []) {
  const parts = [];
  ...
  const units = collectUnits(parsedFiles, "seed", false);
  const resolved = resolveUnits(units, appliedNames);
  const seedSql = renderResolvedUnits(resolved);
  if (seedSql) {
    parts.push(seedSql);
  }
...
export function compileMigrateUp(parsedFiles, appliedNames) {
  const units = collectUnits(parsedFiles, "migrate:up", true);
  const resolved = resolveUnits(units, [...appliedNames]);
  return renderResolvedUnits(resolved);
}

export function compileMigrateDown(parsedFiles, appliedNamesOrderedDesc) {
  ...
  for (const name of appliedNamesOrderedDesc) {
    ...
    parts.push(`DELETE FROM api.migration WHERE name = ${sqlLiteral(name)};\n`);
  }
